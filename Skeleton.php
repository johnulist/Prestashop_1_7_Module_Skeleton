...
class Skeleton extends Module
{
    ...
    {
		$install_sql_file = dirname(__FILE__).'/sql/install.sql';
        return  parent::install() && $this->registerHook('displayBackOfficeHeader') && $this->installTabs($this->config_module['tabs']) && $this->loadSQLFile($install_sql_file);
    }

    public function uninstall()
    {
		$uninstall_sql_file = dirname(__FILE__).'/sql/uninstall.sql';
        return $this->uninstallTabs($this->config_module['tabs']) && $this->unregisterHook('displayBackOfficeHeader') && $this->loadSQLFile($uninstall_sql_file) && parent::uninstall();
    }
